...
@section('content')
    {{-- @dd($location); --}}
    <form action="{{ route('instrument-types.update', ['instrument_type' => $instrument_type]) }}" method="POST">
        @csrf
        @method('PATCH')
        <input type="text" name="instrument_type" value="{{ $instrument_type->instrument_type }}">
        <button type="submit">
            Submit
        </button>
    </form>
